  ...
  socket.on('commit', async (roomId) => {
    ...
    if (!review['isCommit']) {
      review['images'] = review['images'].reduce( ( filtered, el ) => {
        ...
        newEl = newEl.join('/');
        if ( review['content'] !== review['content'].replace(el, newEl) ){
          filtered.push(newEl);
          imgOption = 'copy';
          review['content'] = review['content'].replace(el, newEl);
        }; 
        commitImages({ reviewId: roomId, prevImgPath: el, imgOption, nextImgPath: newEl });
        return filtered;
      }, []);
    }
